...
export default function RegisterPage() {
  ...
  // Handle file upload — sends to backend
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 300 * 1024) {
      setError("Resume must be under 300KB.")
      setLoading(false)
      return
    }
    try {
  ...
  const handleRemove = async () => {
    if (!fileName) return
    try {
      await apiDelete("/api/delete", { fileName })
    } catch {
      // File may not exist on server, that's ok
    }
    setFileName("")
    setFilePath("")
  }

  // Register for interview — calls backend API
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiPost("/api/interviews", { name, fileName, filePath })
      setInterview({
        id: data.id,
  ...
  // Start interview — navigate to playground with interview ID
  const startInterview = () => {
    setIsModalOpen(false)
    router.push(`/playground?interviewId=${interview.id}`)
  }
